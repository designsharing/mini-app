...
/** Flutter 心跳回包数据 */
export interface KeepHeartResponseData {
  appId?: string
  appid?: string
}
...
declare global {
  interface Window {
    chat: {
       ...
       * @example
       * window.chat.sendKeepHeart('app-id', (data) => {
       *   console.log(data)
       * });
       ...
       */
      sendKeepHeart: (
        appIdOrCallback?: string | KeepHeartCallback,
        callback?: KeepHeartCallback
      ) => void
      /**
       * 设置右上角bar位置
       * @param params 请求参数
      ...
      */
      ...
      /**
       * 小程序生命周期探测。
       * getReport/getStatus/restartServer/reloadPage 的 value 传 null；
       * setAutoRepair 的 value 传 boolean。
       */
      miniAppLifecycleProbe: (
        params: MiniAppLifecycleProbeParams,
